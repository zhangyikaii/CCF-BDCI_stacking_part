...
test_dataset = Data.TensorDataset(torch.from_numpy(X_test).double(),
                                 torch.from_numpy(Y_test).double())
...
with torch.no_grad():
    ...
    for dat, target in test_loader:
        df = pd.read_csv('./result/model_0/test.csv')
        print(len(dat))
        dat = dat.to(DEVICE)
        outputs = model(dat.float())
        # print(outputs)
        _, predicted = torch.max(outputs.data, 1)
        df['label'] = predicted
        df[['id', 'label']].to_csv("NNres/test.csv", index=False)
        # total += 1
